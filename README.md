...
- 일정 등록/수정 화면에서 "체크리스트 추가" 버튼을 눌러야만 하위 할 일 입력 UI가 나타납니다(기본은 숨김). 하위 항목은 체크박스 + 텍스트 한 줄로, 완료 시 취소선·흐리게 처리되고 자유롭게 추가/삭제할 수 있습니다.
- 일정 객체에 `subtasks` 배열이 옵셔널로 붙습니다(`{ id, text, completed }[]`). 기존에 저장된 일정처럼 이 필드가 아예 없는 데이터는 `[]`로 취급되어 그대로 동작하고, 저장 시에만 배열이 채워집니다 — 별도 마이그레이션이나 DB 버전업 없이 하위호환됩니다.
- 하위 체크리스트가 있는 일정은 선택한 날짜의 일정 카드에 "N/M 완료" 배지와 얇은 진행률 바가 표시되며, 전부 완료되면 초록색으로 강조됩니다. 이 진행률은 기존 게이미피케이션 점수·스트릭 시스템과는 완전히 분리되어 있어 XP/레벨에 영향을 주지 않습니다.
